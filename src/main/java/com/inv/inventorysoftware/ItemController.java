...
public class ItemController {
    ...
    // POST /api/items/{id}/increment → Add 1 to quantity
...
After: { "id": "M01", "name": "Dell Monitor", "quantity": 11 }
    ...
    // POST /api/items/{id}/decrement → Subtract 1 from quantity
    ...
    // PUT /api/items/{id}/comment → Update item comment
    @PutMapping("/{id}/comment")
    public Item updateComment(@PathVariable String id, @RequestBody CommentRequest commentRequest) {
        Item item = itemRepository.findById(id).orElseThrow(); // fetch item by ID or fail
        item.setComment(commentRequest.getComment()); // update the comment field
        return itemRepository.save(item); // save updated item
    }

    // Helper class to handle the incoming JSON with "comment" key
